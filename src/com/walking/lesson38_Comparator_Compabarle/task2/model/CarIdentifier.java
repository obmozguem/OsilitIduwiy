package com.walking.lesson38_Comparator_Compabarle.task2.model;
...
public class CarIdentifier implements Comparable<CarIdentifier> {
    ...
    @Override
    public int compareTo(CarIdentifier c) {
        if (number.compareTo(c.number) > 0) {
            return 1;
        } else if (number.compareTo(c.number) < 0) {
            return -1;
        } else {
            return year - c.year;
        }
